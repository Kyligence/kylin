...
public class NSparkCubingJob extends DefaultExecutableOnModel {
    ...
    @VisibleForTesting
    public static NSparkCubingJob createIncBuildJob(Set<NDataSegment> segments, //
            Set<LayoutEntity> layouts, String submitter, Set<JobBucket> buckets) {
        ...
        switch (getJobType()) {
            ...
            case SUB_PARTITION_REFRESH:
                for (String id : segmentIds) {
                    ...
                    logger.info("Change partitions [{}] in segment [{}] status to READY cause to cancel job.",
                            partitions, id);
                }
                break;
            default:
