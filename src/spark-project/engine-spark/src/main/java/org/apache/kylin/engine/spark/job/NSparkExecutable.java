...
public class NSparkExecutable extends AbstractExecutable implements ChainedStageExecutable {
    ...
    public void waiteForResourceStart(JobContext context) {
        // mark waiteForResource step start
    ...
    private Map<String, String> getSparkConf(KylinConfig kylinConf) {
        ...
        // Rewrite index planner spark conf.
        rewriteConfOfIndexPlanner(kylinConf, sparkConf);
    ...
    private void rewriteConfOfIndexPlanner(KylinConfig kylinConf, Map<String, String> sparkConf) {
        ...
        if (indexPlannerEnabled) {
            Map<String, String> indexPlannerSparkConf = kylinConf.getIndexPlannerBuildingConfigOverride();
            try {
                String indexPlannerSparkConfStr = JsonUtil.writeValueAsString(indexPlannerSparkConf);
                sparkConf.putAll(indexPlannerSparkConf);
                logger.info("Override spark submit conf using index planner specified sparkConf: {}",
                        indexPlannerSparkConfStr);
