...
import org.apache.kylin.engine.spark.job.step.{InferiorGroup, ParamPropagation, StageExec}
...
abstract class BuildStage(private val jobContext: SegmentJob,
  ...
  protected final val appStatusContext = jobContext.getAppStatusContext
  ...
  private lazy val flatTableStage = params.getFlatTableStage
  private lazy val flatTable: Dataset[Row] = params.getFlatTable
  private lazy val flatTableStats: Statistics = params.getFlatTableStats
  private lazy val sanityResultQueue = Queues.newLinkedBlockingQueue[SanityResult]()
  // thread unsafe
  private lazy val cachedLayoutSanity = params.getCachedLayoutSanity
  // thread unsafe
  private lazy val cachedLayouts = params.getCachedLayouts
  // thread unsafe
  private lazy val cachedIndexInferior = params.getCachedIndexInferior
  ...
  override def getSegmentId: String = segmentId
  ...
  private def flatTableDesc = params.getFlatTableDesc
  ...
  private def getCachedLayout(segment: NDataSegment, layout: LayoutEntity): Dataset[Row] = synchronized {
    cachedLayouts.getOrElseUpdate(layout.getId,
  ...
  private def spanningTree = params.getSpanningTree

  override protected def recordTaskInfo(t: Task): Unit = {
    logInfo(s"Segment $segmentId submit task: ${t.getTaskDesc}")
  }

  protected def buildStats(): Statistics = {
    flatTableStage.gatherStats()
  }
  ...
  protected def buildSanityCache(): Unit = {
    ...
    params.setCachedLayoutSanity(Some(sanityMap))
      ...
      override def process(): Unit = {
        ...
        if (!jobContext.isPartialBuild && copiedSegment.getModel.getStorageType.isV1Storage) {
          copiedSegment.setDimensionRangeInfoMap(
            calDimRange(dataSegment, flatTableStage.getFlatTable)
          )
        }
