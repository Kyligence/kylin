...
import org.apache.kylin.engine.spark.job.step.build.BuildStage
...
abstract class PartitionBuildStage(jobContext: SegmentJob, dataSegment: NDataSegment, params: ParamPropagation)
  extends BuildStage(jobContext, dataSegment, params) with PartitionExec {
  protected final val newBuckets = //
    jobContext.getReadOnlyBuckets.asScala.filter(_.getSegmentId.equals(segmentId)).toSeq

  private lazy val spanningTree: PartitionSpanningTree = params.getPartitionSpanningTree

  private lazy val flatTableDesc: PartitionFlatTableDesc = params.getTableDesc
  private lazy val flatTable: PartitionFlatTableStage = params.getPartitionFlatTable
  ...
  private lazy val cachedPartitions = params.getCachedPartitions

  // Thread unsafe, read only. Partition flat table statistics.
  private lazy val cachedPartitionStats = params.getCachedPartitionStats

  // Thread unsafe
  // [layout, [partition, dataset]]
  private lazy val cachedLayouts = params.getCachedLayoutPartitions

  // Thread unsafe
  // [layout, [partition, sanity]]
  private lazy val cachedLayoutSanity = params.getCachedLayoutPartitionSanity
  ...
  private def getPartitionTasks(segment: NDataSegment, node: PartitionTreeNode): Seq[PartitionBuildTask] = {
    ...
    if (node.parentIsNull) {
      ...
      layouts.map { layout =>
        PartitionBuildTask(layout, node.getPartition, None, cachedPartitions(node.getPartition), sanityCount, segment)
      }
    } else {
      // Build from data layout
      val parentLayout = node.getParent.getLayout
      val parentDS = getCachedPartition(segment, parentLayout, node.getPartition)
  ...
  private def getCachedPartition(segment: NDataSegment, //
                                 layout: LayoutEntity, //
                                 partition: Long): Dataset[Row] = synchronized {
    cachedLayouts.getOrElseUpdate(layout.getId, // or update
  ...
  override protected def buildSanityCache(): Unit = {
    ...
    val taskIter = rootNodes.map { node =>
      ...
      val partitionDS = getCachedPartition(dataSegment, layout, partition)
    ...
    params.setCachedLayoutPartitionSanity(Some(sanityMap))
