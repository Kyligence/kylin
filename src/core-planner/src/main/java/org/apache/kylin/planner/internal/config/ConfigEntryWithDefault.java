...
class ConfigEntryWithDefault<T> extends ConfigEntry<T> {
    ...
    protected ConfigEntryWithDefault(String key, String prependedKey, String prependSeparator,
            List<String> alternatives, T defaultValue, ConfigHelper.Converter<String, T> valueConverter,
    ...
    public String defaultValueString() {
        return stringConverter.convert(defaultValue);
    }

    @Override
    public T readFrom(ConfigReader reader) {
        return readString(reader).map(valueConverter::convert).orElse(defaultValue);
    }
}
