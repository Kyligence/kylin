...
public class NDataLayoutDetailsManagerTest {
    ...
    public void testBasic() {
        ...
        NDataLayoutDetails layoutFragment1 =
                layoutFragmentMgr.getNDataLayoutDetails(dataflow.getId(), layoutEntity.getId());
        ArrayList<Range<Long>> ranges1 = Lists.newArrayList(layoutFragment1.getFragmentRangeSet().asRanges().iterator());
        Assertions.assertEquals(
                Lists.newArrayList(Range.closedOpen(0L, 200L), Range.closedOpen(201L, 300L)),
                ranges1);
        layoutFragmentMgr.updateLayoutDetails(dataflow.getId(), layoutEntity.getId(), (copyForWrite) -> {
            copyForWrite.getFragmentRangeSet().remove(Range.closedOpen(50L, 100L));
        });
        NDataLayoutDetails layoutFragment2 =
                layoutFragmentMgr.getNDataLayoutDetails(dataflow.getId(), layoutEntity.getId());
        ArrayList<Range<Long>> ranges2 = Lists.newArrayList(layoutFragment2.getFragmentRangeSet().asRanges().iterator());
        Assertions.assertEquals(
