...
class PartitionRowIterator(iter: Iterator[Row], parsedSchema: StructType) extends Iterator[Row] {
  ...
  def parseValue(jsonMap: mutable.HashMap[String, String], colName: String, index: Int): Any = {
    ...
    dataType match {
      ...
      case TimestampType => processTimestamp(value)
      case DateType => processDate(value)
      case DecimalType() => BigDecimal(value)
      case _ => value
    }
  }

  def processTimestamp(value: String): Timestamp = {
    new Timestamp(checkTimestamp(value))
  }

  def processDate(value: String): Date = {
    new Date(checkTimestamp(value))
  }

  def checkTimestamp(value: String): Long = {
