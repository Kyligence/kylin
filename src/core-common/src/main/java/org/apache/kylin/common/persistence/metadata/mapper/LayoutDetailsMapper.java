...
import static org.apache.kylin.common.persistence.metadata.mapper.LayoutDetailsDynamicSqlSupport.sqlTable;

public interface LayoutDetailsMapper extends BasicMapper<LayoutDetailsRawResource> {

    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.dataFlowId, sqlTable.layoutId);
    }
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(LayoutDetailsRawResource record, UpdateDSL<UpdateModel> dsl) {
    ...
    @ResultMap("LayoutDetailResult")
    Optional<LayoutDetailsRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "LayoutDetailResult", value = {
            ...
            @Result(column = "layout_id", property = "layoutId", jdbcType = JdbcType.VARCHAR),
    ...
    List<LayoutDetailsRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("LayoutDetailResult")
    List<LayoutDetailsRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
