...
import static org.apache.kylin.common.persistence.metadata.mapper.UserGroupDynamicSqlSupport.sqlTable;
...
public interface UserGroupMapper extends BasicMapper<UserGroupRawResource> {
    @Override
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(UserGroupRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.groupName).equalTo(record::getGroupName);
    }

    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.groupName);
    }
    ...
    int insert(InsertStatementProvider<UserGroupRawResource> insertStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @ResultMap("UserGroupResult")
    Optional<UserGroupRawResource> selectOne(SelectStatementProvider selectStatement);
    ...
    List<UserGroupRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("UserGroupResult")
    List<UserGroupRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
